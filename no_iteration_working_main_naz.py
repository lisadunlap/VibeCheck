...
def propose_vibes(
    df, models, num_proposal_samples=30, num_final_vibes=10, batch_size=5, iterations=1
):
...
"""
    ...
    for iteration in range(iterations):
        ...
        all_differences.append(proposer_df["differences"])
    
        #if else filtering out
        if iteration > 0:
            previous_vibes = all_differences[-2]  # prev
            proposer_df = proposer_df[~proposer_df["differences"].isin(previous_vibes)]
    
    # Combine all differences from all iterations
    combined_differences = [item for sublist in all_differences for item in sublist]
    combined_differences = list(set(combined_differences))  # Remove duplicates
...
def parse_args():
    ...
    parser.add_argument("--data_path", type=str, required=True, help="Path to the CSV file containing model outputs")
    parser.add_argument("--models", nargs="+", required=True, help="Models to compare")
    parser.add_argument("--test", action="store_true", help="Run in test mode")
    parser.add_argument("--position_matters", action="store_true", help="Rerun ranker with different positions")
    parser.add_argument("--proposer_only", action="store_true", help="Only run the proposer")

    # Add arguments for parameters that can be overridden
    parser.add_argument("--num_proposal_samples", type=int, help="Number of samples to use for proposing vibes")
    parser.add_argument("--num_final_vibes", type=int, help="Number of final vibes to use for analysis")
    parser.add_argument("--batch_size", type=int, help="Batch size for processing")
    parser.add_argument("--iterations", type=int, help="Number of iterations for proposing vibes")
    parser.add_argument("--preset_vibe", type=str, help="Specify a preset vibe option")
...
def main(
    ...
    #TODO Jan 22
    #Iteratation (missclassified_df, vibes):
    #   prompt "don't list {vibes}"

    #TODO Jan 22
    #get_misclassified(df, mm_model, pp_model):
    #find samples in df that are misclassified (forward-pass)
    ...
    #TODO
    #another function? return for train_and _evalauate — separate function

    #TODO
    #iteration
    #parser - add argument, default config (not super efficient)
    
    #TODO
    #propose_vibes
    #if else':  if not empty just add files
...
if __name__ == "__main__":
    ...
    for key in ['num_proposal_samples', 'num_final_vibes', 'batch_size', 'iterations', 'preset_vibe', 'project_name']:
        value = getattr(args, key)
        if value is not None:
            config[key] = value

    # Initialize wandb
    wandb.init(project=config.get("project_name", "vibecheck"), name=f"[new]{args.models[0]}_vs_{args.models[1]}", save_code=True)
    ...
    if args.test:
        df = df.sample(100, random_state=42)
    df = df[df["preference"].isin(args.models)].reset_index(drop=True)

    # Check for preset vibe option
    if config["preset_vibe"]:
        print(f"Using preset vibe: {config['preset_vibe']}")
        # Implement logic to filter or load preset vibes here

    # Propose vibes
    vibes = propose_vibes(
        df,
        args.models,
        num_proposal_samples=config["num_proposal_samples"],
    ...
    if args.proposer_only:
        print("Current script proposer_only is set to True.")
        exit()
    ...
    lotus.settings.configure(lm=lm, enable_cache=True)
    if args.test:
        vibe_df = rank_axes(vibes[:3], df, args.models, position_matters=args.position_matters)
    else:
        vibe_df = rank_axes(vibes, df, args.models, position_matters=args.position_matters)
    ...
    # Compute preference alignment
    vibe_df["preference_feature"] = vibe_df["preference"].apply(
        lambda x: get_pref_score(x, args.models)
    )
    vibe_df["pref_score"] = vibe_df["score"] * vibe_df["preference_feature"]
